    ...
    public class fireArrow : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[1] == 45f  && Projectile.owner == Main.myPlayer && Projectile.ai[2] == 0f)
            {
                ...
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position.X, Projectile.position.Y, speedX, speedY, ModContent.ProjectileType<Projectiles.fireArrow>(), (int)(Projectile.damage * 0.75), 0f, Projectile.owner, 0f, 0f, 1f);

                //up
                Vector2 rotateUpSpeed = new Vector2(speedX, speedY).RotatedBy(MathHelper.ToRadians(15));
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position.X, Projectile.position.Y, rotateUpSpeed.X, rotateUpSpeed.Y, ModContent.ProjectileType<Projectiles.fireArrow>(), (int)(Projectile.damage * 0.75), 0f, Projectile.owner, 0f, 0f, 1f);

                //down
                Vector2 rotateDownSpeed = new Vector2(speedX, speedY).RotatedBy(MathHelper.ToRadians(-15));
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position.X, Projectile.position.Y, rotateDownSpeed.X, rotateDownSpeed.Y, ModContent.ProjectileType<Projectiles.fireArrow>(), (int)(Projectile.damage * 0.75), 0f, Projectile.owner, 0f, 0f, 1f);
